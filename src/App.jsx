...
function computeCycleFromLive(d) {
  ...
  if (d.ism_manufacturing   != null) add(Math.max(-1, Math.min(1, (d.ism_manufacturing - 50) / 10)), 0.2, 1.2);
...
function SentimentTab({ live }) {
  ...
  const tiles = [
    { label: "VIX",           value: resolve(L?.vix_latest, "24.3", v => v.toFixed(1)),    prev: "19.1",    signal: "warn", tip: "Options-implied fear gauge. >30 = panic, <15 = complacency. Elevated at 24 — markets pricing near-term uncertainty without full panic.", live: isLive("vix_latest") },
    { label: "Fear & Greed",  value: "32",    prev: "58",     signal: "bad",  tip: "CNN composite of 7 market signals. At 32 = Fear. Contrarian indicator — extreme fear has preceded short-term rallies historically.", live: false },
    { label: "AAII Bull–Bear",value: "–18pts",prev: "+12",    signal: "bad",  tip: "Retail investor sentiment. Net bearish at –18pts. Below –20 is a reliable contrarian buy signal historically.", live: false },
    { label: "Put/Call Ratio",value: "1.18",  prev: "0.87",   signal: "warn", tip: ">1 = more puts than calls. Spikes above 1.3 often mark short-term capitulation lows.", live: false },
...
function FundamentalsTab({ live }) {
  ...
  const tiles = [
    ...
    { label: "Conference Bd LEI",value: "–0.6%",     prev: "–0.2%",   signal:"bad",  tip:"Composite of 10 leading indicators. Sustained decline = recession signal. Has predicted every US recession with ~12 month lead.", live:false },
    { label: "ISM Manufacturing", value: "48.3",      prev: "50.1",    signal:"bad",  tip:"<50 = contraction. Multiple months below 50 signal global industrial cycle deteriorating.", live:false },
