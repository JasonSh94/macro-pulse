...
function SentimentTab({ live }) {
  ...
  const tiles = [
    ...
    { label: "Margin Debt",    value: resolve(L?.margin_debt, "$748B", v => `$${Math.round(v)}B`), prev: "$779B", signal: L?.margin_debt != null ? (L.margin_debt > 900 ? "bad" : L.margin_debt > 750 ? "warn" : "good") : "good", tip: "NYSE debit balances — total margin leverage in the system. Rising margin debt signals elevated systemic fragility; historically peaks lead equity peaks by 3–6 months.", live: isLive("margin_debt") },
...
function ValuationsTab({ live }) {
  ...
  const tiles = [
    { label: "S&P 500 P/E",    value: resolve(L?.sp500_fwd_pe, "21.4×", v => `${v.toFixed(1)}×`), prev: "19.8×", signal: L?.sp500_fwd_pe != null ? (L.sp500_fwd_pe > 22 ? "bad" : L.sp500_fwd_pe > 18 ? "warn" : "good") : "bad", tip: "Trailing 12-month P/E ratio (SPY). Historical avg ~17–18×. Elevated multiple vs. 2%+ real yields implies a thin equity risk premium.", live: isLive("sp500_fwd_pe") },
    { label: "Equity Risk Prem",value: resolve(L?.equity_risk_premium,"1.8%", v => `${v.toFixed(1)}%`), prev: "2.9%", signal: L?.equity_risk_premium != null ? (L.equity_risk_premium < 1 ? "bad" : L.equity_risk_premium < 2.5 ? "warn" : "good") : "bad", tip: "Earnings yield (1/P/E) minus 10Y real yield. Near zero means stocks offer almost no premium over risk-free bonds — stretched valuations relative to rates.", live: isLive("equity_risk_premium") },
